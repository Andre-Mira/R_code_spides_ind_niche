...
holocron_L_geo <- holocron %>% filter(Spp == "L_geo")
holocron_L_ery <- holocron %>% filter(Spp == "L_ery")

# Function to resample and calculate mean for each variable
resample_data <- function(data, n) {
  n_available <- nrow(data)
  n_sample <- 2             # min(n_available/4)  <- Ensure we don't sample more than available
  
  resampled_data <- data %>% sample_n(n_sample, replace = FALSE)
  
  resampled_summary <- resampled_data %>%
    summarise(
      TA_H1_mean = mean(TA_H1),
      TA_H2_mean = mean(TA_H2),
      TA_H3_mean = mean(TA_H3),
      TB_H1_mean = mean(TB_H1),
      TB_H2_mean = mean(TB_H2),
      TB_H3_mean = mean(TB_H3),
      TC_H1_mean = mean(TC_H1),
      TC_H2_mean = mean(TC_H2),
      TC_H3_mean = mean(TC_H3),
      I13C_mean = mean(I13C),
      I15N_mean = mean(I15N)
    )
...
# Number of individuals to resample (set according to species)
n_resample_A_arg <- min(nrow(holocron_A_arg), 13)  # Ensure resample size <= number of available individuals
n_resample_L_arg <- min(nrow(holocron_L_arg), 8)
n_resample_L_geo <- min(nrow(holocron_L_geo), 12)
n_resample_L_ery <- min(nrow(holocron_L_ery), 5)

# Perform 10000 resampling iterations for each species and store results
n_iterations <- 10000

# Safely resample data for each species
safe_resample <- function(data, n_resample, n_iterations) {
  replicate(n_iterations, {
    if (nrow(data) >= n_resample) {
      resample_data(data, n_resample)
    } else {
      NULL  # Skip this iteration if not enough data
    }
  }, simplify = FALSE) %>%
    bind_rows()  # Combine the results into one data frame
}

# Resampling for each species
resampling_results_A_arg <- safe_resample(holocron_A_arg, n_resample_A_arg, n_iterations)
resampling_results_L_arg <- safe_resample(holocron_L_arg, n_resample_L_arg, n_iterations)
resampling_results_L_geo <- safe_resample(holocron_L_geo, n_resample_L_geo, n_iterations)
resampling_results_L_ery <- safe_resample(holocron_L_ery, n_resample_L_ery, n_iterations)

# Combine all resampling results into one data frame
resampling_results <- bind_rows(
  resampling_results_A_arg %>% mutate(Spp = "A_arg"),
  resampling_results_L_arg %>% mutate(Spp = "L_arg"),
  resampling_results_L_geo %>% mutate(Spp = "L_geo"),
  resampling_results_L_ery %>% mutate(Spp = "L_ery")
)

# Calculate original (non-resampled) means for each species
...
# Analyze results of resampling to compare with original values
resampling_means <- resampling_results %>%
  group_by(Spp) %>%
  summarise(
    TA_H1_mean_resampled = mean(TA_H1_mean, na.rm = TRUE),
    TA_H2_mean_resampled = mean(TA_H2_mean, na.rm = TRUE),
    TA_H3_mean_resampled = mean(TA_H3_mean, na.rm = TRUE),
    TB_H1_mean_resampled = mean(TB_H1_mean, na.rm = TRUE),
    TB_H2_mean_resampled = mean(TB_H2_mean, na.rm = TRUE),
    TB_H3_mean_resampled = mean(TB_H3_mean, na.rm = TRUE),
    TC_H1_mean_resampled = mean(TC_H1_mean, na.rm = TRUE),
    TC_H2_mean_resampled = mean(TC_H2_mean, na.rm = TRUE),
    TC_H3_mean_resampled = mean(TC_H3_mean, na.rm = TRUE),
    I13C_mean_resampled = mean(I13C_mean, na.rm = TRUE),
    I15N_mean_resampled = mean(I15N_mean, na.rm = TRUE)
  )

# Compare original means with resampled means
comparison <- original_means %>%
  left_join(resampling_means, by = "Spp") %>%
  mutate(
    TA_H1_diff = TA_H1_mean_resampled - TA_H1_mean,
    TA_H2_diff = TA_H2_mean_resampled - TA_H2_mean,
    TA_H3_diff = TA_H3_mean_resampled - TA_H3_mean,
    TB_H1_diff = TB_H1_mean_resampled - TB_H1_mean,
    TB_H2_diff = TB_H2_mean_resampled - TB_H2_mean,
    TB_H3_diff = TB_H3_mean_resampled - TB_H3_mean,
    TC_H1_diff = TC_H1_mean_resampled - TC_H1_mean,
    TC_H2_diff = TC_H2_mean_resampled - TC_H2_mean,
    TC_H3_diff = TC_H3_mean_resampled - TC_H3_mean,
    I13C_diff = I13C_mean_resampled - I13C_mean,
    I15N_diff = I15N_mean_resampled - I15N_mean
  )

# View the comparison of original means and resampled means
print(comparison)



# Visualize the distribution of resampled means versus the original mean for T1
ggplot(resampling_results, aes(x = TA_H1_mean, fill = Spp)) +
  geom_density(alpha = 0.5) +
  geom_vline(data = original_means, aes(xintercept = TA_H1_mean, color = Spp), linetype = "dashed", size = 1) +
  labs(title = "Density Plot of Resampled TA_H1 Means vs Original TA_H1 Means",
       x = "TA_H1 Mean",
       y = "Density") +
  theme_minimal() +
...
# Perform t-test comparing original and resampled means for each species and temperature metric

# Helper function to perform a t-test and return the result
perform_t_test <- function(resampled_col, original_mean) {
  t.test(resampled_col, mu = original_mean)
}

# Applying the t-test for TA_H1 for each species
t_test_TA_H1_A_arg <- perform_t_test(resampling_results_A_arg$TA_H1_mean, original_means$TA_H1_mean[original_means$Spp == "A_arg"])
t_test_TA_H1_L_arg <- perform_t_test(resampling_results_L_arg$TA_H1_mean, original_means$TA_H1_mean[original_means$Spp == "L_arg"])
t_test_TA_H1_L_geo <- perform_t_test(resampling_results_L_geo$TA_H1_mean, original_means$TA_H1_mean[original_means$Spp == "L_geo"])
t_test_TA_H1_L_ery <- perform_t_test(resampling_results_L_ery$TA_H1_mean, original_means$TA_H1_mean[original_means$Spp == "L_ery"])

# Print t-test results
print("T-test results for A_arg (TA_H1):")
print(t_test_TA_H1_A_arg)

#OK!

print("T-test results for L_arg (TA_H1):")
print(t_test_TA_H1_L_arg)

#OK!

print("T-test results for L_geo (TA_H1):")
print(t_test_TA_H1_L_geo)

#OK!

print("T-test results for L_ery (TA_H1):")
print(t_test_TA_H1_L_ery)

#OK!



# Applying the t-test for TA_H2 for each species
t_test_TA_H2_A_arg <- perform_t_test(resampling_results_A_arg$TA_H2_mean, original_means$TA_H2_mean[original_means$Spp == "A_arg"])
t_test_TA_H2_L_arg <- perform_t_test(resampling_results_L_arg$TA_H2_mean, original_means$TA_H2_mean[original_means$Spp == "L_arg"])
t_test_TA_H2_L_geo <- perform_t_test(resampling_results_L_geo$TA_H2_mean, original_means$TA_H2_mean[original_means$Spp == "L_geo"])
t_test_TA_H2_L_ery <- perform_t_test(resampling_results_L_ery$TA_H2_mean, original_means$TA_H2_mean[original_means$Spp == "L_ery"])

# Print t-test results
print("T-test results for A_arg (TA_H2):")
print(t_test_TA_H2_A_arg)

#OK!

print("T-test results for L_arg (TA_H2):")
print(t_test_TA_H2_L_arg)

#OK!

print("T-test results for L_geo (TA_H2):")
print(t_test_TA_H2_L_geo)

#OK!

print("T-test results for L_ery (TA_H2):")
print(t_test_TA_H2_L_ery)
...
# Applying the t-test for TA_H3 for each species
t_test_TA_H3_A_arg <- perform_t_test(resampling_results_A_arg$TA_H3_mean, original_means$TA_H3_mean[original_means$Spp == "A_arg"])
t_test_TA_H3_L_arg <- perform_t_test(resampling_results_L_arg$TA_H3_mean, original_means$TA_H3_mean[original_means$Spp == "L_arg"])
t_test_TA_H3_L_geo <- perform_t_test(resampling_results_L_geo$TA_H3_mean, original_means$TA_H3_mean[original_means$Spp == "L_geo"])
t_test_TA_H3_L_ery <- perform_t_test(resampling_results_L_ery$TA_H3_mean, original_means$TA_H3_mean[original_means$Spp == "L_ery"])

# Print t-test results
print("T-test results for A_arg (TA_H3):")
print(t_test_TA_H3_A_arg)

#OK!

print("T-test results for L_arg (TA_H3):")
print(t_test_TA_H3_L_arg)

#OK!

print("T-test results for L_geo (TA_H3):")
print(t_test_TA_H3_L_geo)

#OK!

print("T-test results for L_ery (TA_H3):")
print(t_test_TA_H3_L_ery)
...
# Applying the t-test for TB_H1 for each species
t_test_TB_H1_A_arg <- perform_t_test(resampling_results_A_arg$TB_H1_mean, original_means$TB_H1_mean[original_means$Spp == "A_arg"])
t_test_TB_H1_L_arg <- perform_t_test(resampling_results_L_arg$TB_H1_mean, original_means$TB_H1_mean[original_means$Spp == "L_arg"])
t_test_TB_H1_L_geo <- perform_t_test(resampling_results_L_geo$TB_H1_mean, original_means$TB_H1_mean[original_means$Spp == "L_geo"])
t_test_TB_H1_L_ery <- perform_t_test(resampling_results_L_ery$TB_H1_mean, original_means$TB_H1_mean[original_means$Spp == "L_ery"])

# Print t-test results
print("T-test results for A_arg (TB_H1):")
print(t_test_TB_H1_A_arg)

#OK!

print("T-test results for L_arg (TB_H1):")
print(t_test_TB_H1_L_arg)

#OK!

print("T-test results for L_geo (TB_H1):")
print(t_test_TB_H1_L_geo)

#OK!

print("T-test results for L_ery (TB_H1):")
print(t_test_TB_H1_L_ery)
...
# Applying the t-test for TB_H2 for each species
t_test_TB_H2_A_arg <- perform_t_test(resampling_results_A_arg$TB_H2_mean, original_means$TB_H2_mean[original_means$Spp == "A_arg"])
t_test_TB_H2_L_arg <- perform_t_test(resampling_results_L_arg$TB_H2_mean, original_means$TB_H2_mean[original_means$Spp == "L_arg"])
t_test_TB_H2_L_geo <- perform_t_test(resampling_results_L_geo$TB_H2_mean, original_means$TB_H2_mean[original_means$Spp == "L_geo"])
t_test_TB_H2_L_ery <- perform_t_test(resampling_results_L_ery$TB_H2_mean, original_means$TB_H2_mean[original_means$Spp == "L_ery"])

# Print t-test results
print("T-test results for A_arg (TB_H2):")
print(t_test_TB_H2_A_arg)

#OK!

print("T-test results for L_arg (TB_H2):")
print(t_test_TB_H2_L_arg)

#OK!

print("T-test results for L_geo (TB_H2):")
print(t_test_TB_H2_L_geo)

#OK!

print("T-test results for L_ery (TB_H2):")
print(t_test_TB_H2_L_ery)
...
# Applying the t-test for TB_H3 for each species
t_test_TB_H3_A_arg <- perform_t_test(resampling_results_A_arg$TB_H3_mean, original_means$TB_H3_mean[original_means$Spp == "A_arg"])
t_test_TB_H3_L_arg <- perform_t_test(resampling_results_L_arg$TB_H3_mean, original_means$TB_H3_mean[original_means$Spp == "L_arg"])
t_test_TB_H3_L_geo <- perform_t_test(resampling_results_L_geo$TB_H3_mean, original_means$TB_H3_mean[original_means$Spp == "L_geo"])
t_test_TB_H3_L_ery <- perform_t_test(resampling_results_L_ery$TB_H3_mean, original_means$TB_H3_mean[original_means$Spp == "L_ery"])

# Print t-test results
print("T-test results for A_arg (TB_H3):")
print(t_test_TB_H3_A_arg)

#OK!

print("T-test results for L_arg (TB_H3):")
print(t_test_TB_H3_L_arg)

#OK!

print("T-test results for L_geo (TB_H3):")
print(t_test_TB_H3_L_geo)

#OK!

print("T-test results for L_ery (TB_H3):")
print(t_test_TB_H3_L_ery)
...
# Applying the t-test for TC_H1 for each species
t_test_TC_H1_A_arg <- perform_t_test(resampling_results_A_arg$TC_H1_mean, original_means$TC_H1_mean[original_means$Spp == "A_arg"])
t_test_TC_H1_L_arg <- perform_t_test(resampling_results_L_arg$TC_H1_mean, original_means$TC_H1_mean[original_means$Spp == "L_arg"])
t_test_TC_H1_L_geo <- perform_t_test(resampling_results_L_geo$TC_H1_mean, original_means$TC_H1_mean[original_means$Spp == "L_geo"])
t_test_TC_H1_L_ery <- perform_t_test(resampling_results_L_ery$TC_H1_mean, original_means$TC_H1_mean[original_means$Spp == "L_ery"])

# Print t-test results
print("T-test results for A_arg (TC_H1):")
print(t_test_TC_H1_A_arg)

#OK!

print("T-test results for L_arg (TC_H1):")
print(t_test_TC_H1_L_arg)

#OK!

print("T-test results for L_geo (TC_H1):")
print(t_test_TC_H1_L_geo)

#OK!

print("T-test results for L_ery (TC_H1):")
print(t_test_TC_H1_L_ery)
...
# Applying the t-test for TC_H2 for each species
t_test_TC_H2_A_arg <- perform_t_test(resampling_results_A_arg$TC_H2_mean, original_means$TC_H2_mean[original_means$Spp == "A_arg"])
t_test_TC_H2_L_arg <- perform_t_test(resampling_results_L_arg$TC_H2_mean, original_means$TC_H2_mean[original_means$Spp == "L_arg"])
t_test_TC_H2_L_geo <- perform_t_test(resampling_results_L_geo$TC_H2_mean, original_means$TC_H2_mean[original_means$Spp == "L_geo"])
t_test_TC_H2_L_ery <- perform_t_test(resampling_results_L_ery$TC_H2_mean, original_means$TC_H2_mean[original_means$Spp == "L_ery"])

# Print t-test results
print("T-test results for A_arg (TC_H2):")
print(t_test_TC_H2_A_arg)

#OK!

print("T-test results for L_arg (TC_H2):")
print(t_test_TC_H2_L_arg)

#OK!

print("T-test results for L_geo (TC_H2):")
print(t_test_TC_H2_L_geo)

#OK!

print("T-test results for L_ery (TC_H2):")
print(t_test_TC_H2_L_ery)
...
# Applying the t-test for TC_H3 for each species
t_test_TC_H3_A_arg <- perform_t_test(resampling_results_A_arg$TC_H3_mean, original_means$TC_H3_mean[original_means$Spp == "A_arg"])
t_test_TC_H3_L_arg <- perform_t_test(resampling_results_L_arg$TC_H3_mean, original_means$TC_H3_mean[original_means$Spp == "L_arg"])
t_test_TC_H3_L_geo <- perform_t_test(resampling_results_L_geo$TC_H3_mean, original_means$TC_H3_mean[original_means$Spp == "L_geo"])
t_test_TC_H3_L_ery <- perform_t_test(resampling_results_L_ery$TC_H3_mean, original_means$TC_H3_mean[original_means$Spp == "L_ery"])

# Print t-test results
print("T-test results for A_arg (TC_H3):")
print(t_test_TC_H3_A_arg)

#OK!

print("T-test results for L_arg (TC_H3):")
print(t_test_TC_H3_L_arg)

#OK!

print("T-test results for L_geo (TC_H3):")
print(t_test_TC_H3_L_geo)

#OK!

print("T-test results for L_ery (TC_H3):")
print(t_test_TC_H3_L_ery)

#OK!



# Applying the t-test for I13C for each species
t_test_I13C_A_arg <- perform_t_test(resampling_results_A_arg$I13C_mean, original_means$I13C_mean[original_means$Spp == "A_arg"])
t_test_I13C_L_arg <- perform_t_test(resampling_results_L_arg$I13C_mean, original_means$I13C_mean[original_means$Spp == "L_arg"])
t_test_I13C_L_geo <- perform_t_test(resampling_results_L_geo$I13C_mean, original_means$I13C_mean[original_means$Spp == "L_geo"])
t_test_I13C_L_ery <- perform_t_test(resampling_results_L_ery$I13C_mean, original_means$I13C_mean[original_means$Spp == "L_ery"])

# Print t-test results
print("T-test results for A_arg (I13C):")
print(t_test_I13C_A_arg)

#OK!

print("T-test results for L_arg (I13C):")
print(t_test_I13C_L_arg)

#OK!

print("T-test results for L_geo (I13C):")
print(t_test_I13C_L_geo)

#OK!

print("T-test results for L_ery (I13C):")
print(t_test_I13C_L_ery)
...
ggplot(resampling_results, aes(x = I13C_mean, fill = Spp)) +
  geom_density(alpha = 0.5) +
  geom_vline(data = original_means, aes(xintercept = I13C_mean, color = Spp), linetype = "dashed", size = 1) +
  labs(title = "Density Plot of Resampled TA_H1 Means vs Original TA_H1 Means",
       x = "I13C Mean",
       y = "Density") +
  theme_minimal() +
...
# Applying the t-test for I15N for each species
t_test_I15N_A_arg <- perform_t_test(resampling_results_A_arg$I15N_mean, original_means$I15N_mean[original_means$Spp == "A_arg"])
t_test_I15N_L_arg <- perform_t_test(resampling_results_L_arg$I15N_mean, original_means$I15N_mean[original_means$Spp == "L_arg"])
t_test_I15N_L_geo <- perform_t_test(resampling_results_L_geo$I15N_mean, original_means$I15N_mean[original_means$Spp == "L_geo"])
t_test_I15N_L_ery <- perform_t_test(resampling_results_L_ery$I15N_mean, original_means$I15N_mean[original_means$Spp == "L_ery"])

# Print t-test results
print("T-test results for A_arg (I15N):")
print(t_test_I15N_A_arg)

#OK!

print("T-test results for L_arg (I15N):")
print(t_test_I15N_L_arg)

#OK!

print("T-test results for L_geo (I15N):")
print(t_test_I15N_L_geo)

#OK!

print("T-test results for L_ery (I15N):")
print(t_test_I15N_L_ery)
...
# Visualize the distribution of resampled means versus the original mean for T1
ggplot(resampling_results, aes(x = I15N_mean, fill = Spp)) +
  geom_density(alpha = 0.5) +
  geom_vline(data = original_means, aes(xintercept = I15N_mean, color = Spp), linetype = "dashed", size = 1) +
  labs(title = "Density Plot of Resampled TA_H1 Means vs Original TA_H1 Means",
       x = "I15N Mean",
       y = "Density") +
  theme_minimal() +
